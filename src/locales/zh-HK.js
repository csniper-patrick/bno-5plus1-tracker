export default {
  ...
  instruction: {
    ...
    step2: {
      ...
      point2: '只有整天完全不在英國的完整日曆天才計入離境天數（離英日及抵港日不計入）。',
    ...
    faq: {
      title: '常見問題 FAQ',
      q1: '180日滾動規則是如何計算的？',
      a1: '英國內政部會在 5 年 qualifying period 內的任何一個 12 個月滾動區間進行審核。本工具會逐日檢查所有 365 天區間，確保沒有任何一個區間超過 180 天。',
      q2: '離英日和返英日會算作離境天數嗎？',
      a2: '不會。根據內政部指引，只要該天有任何一部分時間身在英國（包括離境當天及入境當天），均不算作完整的離境天數。',
      q3: '我的個人資料會否傳送到外部伺服器？',
      a3: '絕對不會！BNO 5+1 Tracker 是 100% 本地運行的網頁應用，所有離境紀錄與文件均存於本地裝置。',
    },
  },
}
